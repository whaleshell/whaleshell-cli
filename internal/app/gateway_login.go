...
func (a *App) GatewayLoginInteractive(token string) error {
	...
	if token == "" {
		token = strings.TrimSpace(os.Getenv("OSG_GATEWAY_TOKEN"))
	}
	if token != "" {
	...
	if issuer != "" {
		if clientID == "" {
			return fmt.Errorf("gateway login: OIDC issuer set but client_id missing (gateway add --oidc-client-id or OSG_OIDC_CLIENT_ID on gateway)")
		}
	...
	if err != nil {
		...
		defer cancel()
		tok, err = gatewayclient.New(u).AuthLogin(ctx)
		if err != nil {
...
func (a *App) gatewayLoginBrowserLocal(gatewayURL string) (string, error) {
	...
	srv := &http.Server{
		Handler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			fmt.Fprint(w, "osg gateway login ok — you can close this tab")
			ch <- result{token: tok}
		}),
