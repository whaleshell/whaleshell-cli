...
// Install copies this binary into ~/.local/share/osg/bin/osg and symlinks
// ~/.local/bin/osg so the CLI is available from any directory.
func (a *App) Install(opt InstallOpts) error {
	...
	shareBin := filepath.Join(home, ".local", "share", "osg", "bin")
	dst := filepath.Join(shareBin, "osg")
	linkDir := filepath.Join(home, ".local", "bin")
	link := filepath.Join(linkDir, "osg")
...
func copyFileAtomic(src, dst string) error {
	...
	tmp, err := os.CreateTemp(filepath.Dir(dst), "osg-install-*")
	if err != nil {
		return err
	}
	tmpName := tmp.Name()
