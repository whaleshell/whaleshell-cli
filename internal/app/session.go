...
// Order: App URL override, OPENSHELL_GATEWAY/OSG_GATEWAY_URL, -g name, config current.
func (a *App) currentGatewayURL() (string, error) {
	if a != nil && a.GatewayURLOverride != "" {
		return strings.TrimRight(a.GatewayURLOverride, "/"), nil
	}
	if v := firstNonEmptyEnv("OPENSHELL_GATEWAY", "OSG_GATEWAY_URL"); v != "" {
		...
		return "", fmt.Errorf("gateway %q from env not in config (osg gateway add)", v)
	}
...
func (a *App) gatewayClient() (*gatewayclient.Client, error) {
	...
	if u == "" {
		return nil, fmt.Errorf("no gateway selected (osg gateway ensure|add|select)")
	}
