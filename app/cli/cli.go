// Package cli is the thin cobra-facing layer for the whaleshell binary.
package cli
...
import (
	...
	tuipkg "github.com/whaleshell/whaleshell-cli/internal/tui"
	"golang.org/x/term"
)
...
func execute(a *service.App, args []string) error {
	...
	case "proxy":
		// Internal: Docker sidecar entrypoint (`/whaleshell/whaleshell proxy --listen … --policy …`).
...
func runCompletions(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: whaleshell completions <bash|zsh|fish|powershell>")
	}
...
func runProvider(a *service.App, args []string) error {
	...
	case "update":
		if len(args) < 2 {
			return fmt.Errorf("usage: whaleshell provider update <name> [--from-existing|--credential KEY[=VALUE]]")
		}
		name := args[1]
		fromExisting := false
	...
	case "get":
		if len(args) < 2 {
			return fmt.Errorf("usage: whaleshell provider get <name>")
		}
		return a.ProviderGet(args[1])
	case "delete", "rm":
		if len(args) < 2 {
			return fmt.Errorf("usage: whaleshell provider delete <name>")
		}
		return a.ProviderDelete(args[1])
	case "refresh":
		if len(args) < 2 {
			return fmt.Errorf("usage: whaleshell provider refresh <name>|status|configure|rotate|delete …")
		}
		switch args[1] {
		case "status":
			if len(args) < 3 {
				return fmt.Errorf("usage: whaleshell provider refresh status <name>")
			}
			return a.ProviderRefreshStatus(args[2])
		case "configure":
			if len(args) < 3 {
				return fmt.Errorf("usage: whaleshell provider refresh configure <name> --credential-key K --strategy S")
			}
		...
		case "rotate":
			if len(args) < 3 {
				return fmt.Errorf("usage: whaleshell provider refresh rotate <name> --credential-key K")
			}
		...
		case "delete":
			if len(args) < 3 {
				return fmt.Errorf("usage: whaleshell provider refresh delete <name> --credential-key K")
			}
			key, err := providerflags.ParseRefreshKey(args[2], args[3:])
			if err != nil {
				return err
	...
	case "effective":
		if len(args) < 2 {
			return fmt.Errorf("usage: whaleshell provider effective <sandbox>")
		}
...
func runInstall(a *service.App, args []string) error {
	...
	for i := 0; i < len(args); i++ {
		...
		case "--force":
			opt.Force = true
		default:
			return fmt.Errorf("unknown flag %q (usage: whaleshell install [--force])", args[i])
		}
	}
...
func runGateway(a *service.App, args []string) error {
	...
	case "remove", "rm", "delete":
		...
		if name == "" {
			return fmt.Errorf("usage: whaleshell gateway remove [name]")
		}
...
func runWorkspace(a *service.App, args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: whaleshell workspace create|get|list|delete|member …")
	}
	switch args[0] {
	case "create":
		name := ""
		...
		if name == "" {
			return fmt.Errorf("usage: whaleshell workspace create --name NAME")
		}
		return a.WorkspaceCreate(name)
	case "get":
		if len(args) < 2 {
			return fmt.Errorf("usage: whaleshell workspace get <name>")
		}
	...
	case "delete", "rm":
		if len(args) < 2 {
			return fmt.Errorf("usage: whaleshell workspace delete <name>")
		}
		return a.WorkspaceDelete(args[1])
	case "member":
		if len(args) < 2 {
			return fmt.Errorf("usage: whaleshell workspace member add|remove|list …")
		}
		switch args[1] {
		case "add":
		...
		case "list", "ls":
			...
			if ws == "" {
				return fmt.Errorf("usage: whaleshell workspace member list --workspace NAME")
			}
...
func parseWorkspaceMemberFlags(args []string, needRole bool) (ws, subject, role string, err error) {
	...
	if ws == "" || subject == "" {
		return "", "", "", fmt.Errorf("usage: whaleshell workspace member add --workspace NAME --subject SUBJECT [--role user|admin]")
	}
...
func runSettings(a *service.App, args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: whaleshell settings get|set|delete …")
	}
	switch args[0] {
	case "get":
		name := ""
	...
	case "delete", "rm":
		...
		if key == "" {
			return fmt.Errorf("usage: whaleshell settings delete --key KEY [--global]")
		}
...
func runForward(a *service.App, args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: whaleshell forward start|stop|list …")
	}
	switch args[0] {
	case "start":
	...
	case "service":
		return fmt.Errorf("forward service: use whaleshell service expose")
	default:
...
func runService(a *service.App, args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: whaleshell service expose|list|get|delete …")
	}
	switch args[0] {
	case "expose":
	...
	case "get":
		if len(args) < 2 {
			return fmt.Errorf("usage: whaleshell service get <sandbox> [service]")
		}
		name := "default"
		if len(args) > 2 {
			name = args[2]
	...
	case "delete", "rm":
		if len(args) < 2 {
			return fmt.Errorf("usage: whaleshell service delete <sandbox> [service]")
		}
		name := "default"
		if len(args) > 2 {
			name = args[2]
...
func runRule(a *service.App, args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: whaleshell rule get|approve|reject|history|clear …")
	}
	switch args[0] {
	case "get", "list", "ls":
		status := ""
...
func parseRuleAction(args []string) (id, reason string, err error) {
	...
	if id == "" {
		return "", "", fmt.Errorf("usage: whaleshell rule approve|reject [--chunk-id] ID [--reason TEXT]")
	}
...
func runLogs(a *service.App, args []string) error {
	...
	if !opt.All && len(opt.Names) == 0 {
		return fmt.Errorf("usage: whaleshell logs <name> [--tail] [-n N] [--since 5m] [--source sandbox] [--level warn]")
	}
...
func runInference(a *service.App, args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: whaleshell inference get|set|update|list|show|local")
	}
	switch args[0] {
	case "get":
...
func runPolicy(a *service.App, args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: whaleshell policy get|set|update|list|delete|check …")
	}
	switch args[0] {
	case "check": // whaleshell-only validation helper
	...
	case "list", "ls":
		...
		if name == "" {
			return fmt.Errorf("usage: whaleshell policy list <sandbox>")
		}
	...
	case "delete", "rm":
		...
		if !global {
			return fmt.Errorf("usage: whaleshell policy delete --global")
		}
...
func runPolicyUpdate(a *service.App, args []string) error {
	if len(args) < 1 {
		return fmt.Errorf("usage: whaleshell policy update <sandbox> --add-endpoint SPEC [--add-allow SPEC] [--add-deny SPEC] [--binary PATH] [--wait]")
	}
	name := args[0]
	wait := false
...
func runSandbox(a *service.App, args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: whaleshell sandbox create|list|get|stop|start|delete|exec|connect|upload|download|ssh-config|provider")
	}
	switch args[0] {
	case "create":
	...
	case "stop":
		if len(args) < 2 {
			return fmt.Errorf("usage: whaleshell sandbox stop <name>")
		}
		return a.SandboxStop(args[1])
	case "start":
		if len(args) < 2 {
			return fmt.Errorf("usage: whaleshell sandbox start <name>")
		}
		return a.SandboxStart(args[1])
	case "rm", "remove", "delete":
		if len(args) < 2 {
			return fmt.Errorf("usage: whaleshell sandbox delete <name>")
		}
	...
	case "provider":
		if len(args) < 2 {
			return fmt.Errorf("usage: whaleshell sandbox provider list|attach|detach …")
		}
		switch args[1] {
		case "list", "ls":
			if len(args) < 3 {
				return fmt.Errorf("usage: whaleshell sandbox provider list <sandbox>")
			}
			return a.SandboxProviderList(args[2])
		case "attach":
			if len(args) < 4 {
				return fmt.Errorf("usage: whaleshell sandbox provider attach <sandbox> <provider>")
			}
			return a.ProviderAttach(args[2], args[3])
		case "detach":
			if len(args) < 4 {
				return fmt.Errorf("usage: whaleshell sandbox provider detach <sandbox> <provider>")
			}
...
func runSandboxTemplate(a *service.App, args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: whaleshell sandbox template create|list|get|delete …")
	}
	switch args[0] {
	case "create":
	...
	case "get":
		if len(args) < 2 {
			return fmt.Errorf("usage: whaleshell sandbox template get <name>")
		}
		return a.TemplateGet(args[1])
	case "delete", "rm":
		if len(args) < 2 {
			return fmt.Errorf("usage: whaleshell sandbox template delete <name>")
		}
...
func runTerm(a *service.App, _ []string) error {
	if !term.IsTerminal(int(os.Stdin.Fd())) {
		return fmt.Errorf("whaleshell term: requires an interactive terminal")
	}
...
// Usage: whaleshell proxy --listen ADDR --policy PATH [--ca-out PATH]
func runProxy(a *service.App, args []string) error {
	...
	for i := 0; i < len(args); i++ {
		...
		case "--log-dir":
			...
			opt.LogDir = args[i]
		case "--help", "-h":
			fmt.Fprintln(os.Stderr, "usage: whaleshell proxy --listen ADDR --policy PATH [--ca-out PATH] [--gateway URL] [--sandbox NAME]")
			return nil
		default:
			if strings.HasPrefix(args[i], "-") {
	...
	if strings.TrimSpace(opt.Policy) == "" {
		return fmt.Errorf("usage: whaleshell proxy --listen ADDR --policy PATH [--ca-out PATH]")
	}
	return a.Proxy(opt)
}
